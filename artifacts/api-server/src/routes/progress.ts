...
router.get("/progress", async (req, res) => {
  ...
  const [{ data: profiles }, { data: weightEntries }, { data: weeklyEvents }] = await Promise.all([
    ...
    db.from("weight_entries").select("date, weight_kg").eq("user_id", req.user.id).order("date", { ascending: true }),
  ...
  res.json(GetProgressResponse.parse({
    ...
    weightHistory: entries.map((e: any) => ({ date: e.date, weightKg: e.weight_kg })),
  }));
});
...
router.post("/progress", async (req, res) => {
  ...
  if (existing) {
    await db.from("weight_entries").update({ weight_kg: body.weightKg }).eq("user_id", req.user.id).eq("date", today);
  } else {
    await db.from("weight_entries").insert({ user_id: req.user.id, date: today, weight_kg: body.weightKg });
  }
