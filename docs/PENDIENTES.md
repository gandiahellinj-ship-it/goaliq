...
**VERIFICADO EN PRODUCCIÓN (29/07):** los 4 arreglos se promocionaron a `main` (cherry-pick, `.replit` de producción preservado) y se desplegaron; José confirmó visualmente que la foto coincide con el plato (p.ej. "Semillas de girasol y queso fresco" sale bien).

### Fotos de plato — coherencia del emplatado + velocidad (29/07, en `staging`)
- **Coherencia ARREGLADA** (rama `feature/fotos-emplatado-coherente`, en `staging`): el prompt ahora pide UN plato terminado y cohesionado (cremas/líquidos integrados como salsa, NUNCA en cuenco aparte; sin cuencos/ramekins ni ingredientes sueltos). Arregla que la "crema de leche" saliera como cuenco blanco separado. **Verificado generando 2 imágenes reales** (ternera con crema como salsa; salmón coherente). Concurrencia 3→6.
- **⚠️ Al promocionar a `main`:** después de desplegar, **VACIAR la caché** `dish_images` (p.ej. `DELETE FROM public.dish_images;` en el SQL Editor de producción) para que las fotos ya cacheadas se regeneren con el prompt nuevo (la `cache_key` NO depende del prompt, así que sin purga se quedarían las viejas).
- **Velocidad (pendiente, mejora mayor):** cada foto tarda ~20s (latencia de Gemini, no reducible por código). La concurrencia ayuda al paralelo, pero para que sean INSTANTÁNEAS al abrir la app hay que **pre-generarlas al crear el plan** (en 2º plano). Tarea futura.
- Mejora futura opcional: que la generación del plan rellene `visual_ref` con una descripción visual de verdad (no la cantidad) — hoy se ignora `visual_ref`.
